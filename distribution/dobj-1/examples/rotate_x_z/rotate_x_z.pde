...
class MyDocumentObject extends DObject {
  ...
  void onMouseClick( CatchableMouseEvent e ) {
    ...
    animate( this.z_angle, this.z_angle + 180, 500, new Lambda<Float>() {
      public void run( Float angle ) {
        setZAngle( Math.round( angle ) );
      }
    } ).then( this.x_angle, this.x_angle + 180, 500, new Lambda<Float>() {
      public void run( Float angle ) {
        setXAngle( Math.round( angle ) );
      }
    } ).run();
  }
}
